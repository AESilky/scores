...
/**
 * @file core1_main.h
 * @defgroup scores_core1 scores_core1
 * Core 1 supervisor and main entry point.
 *
 * NOTE: Fifo use
 * scores uses the Fifo for base (core (ha!)) level communication between
 ...
 *
 * scores acknowledges the warnings, and will take care to assure that the Fifo and
 * the scores code_seq is in an appropriate state when operations are performed that will
 ...
 *
 * scores runs the User Interface (UI, aka Frontend/FE) on Core 1, while it
