...
#define _INPUT_BUF_SIZE_        512 // Make this a power of 2
...
cmt_msg_t stdio_char_ready_msg = { MSG_STDIO_CHAR_READY };
...
void stdio_chars_read(cmt_msg_t* msg) {
    // Nothing important in the msg
...
/**
 * @brief Callback function that is registered with the STDIO handler to be
 *        notified when characters become available.
 *
 * @param param Value that is passed to us that we registered with.
 */
static void _stdio_chars_available(void *param) {
    // This is received in an interrupt handler. When using USB, the character
    // can't be read from within this method. Therefore, we post a message
    // indicating that character(s) is/are available. That allows us to read
    // the characters in a normal thread rather than the ISR.
    //
    // The message is handled by stdio_chars_read() (above)
    //
    postBEMsgNoWait(&stdio_char_ready_msg);
}
...
text_position_t term_get_cursor_position(void) {
    char buf[16];
    text_position_t pos = {-1,-1};
...
void term_module_init() {
    ...
    term_reset();
    sleep_ms(500); // Allow the terminal to reset.
